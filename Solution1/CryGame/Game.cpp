...
//	- November 2016: Modified by Szczekus
//
//////////////////////////////////////////////////////////////////////
...
{
	return m_pServer?m_pServer->m_pISystem:m_pClient?m_pClient->m_pISystem:NULL;
}
...
{	
  ...
  m_pSystem->GetILog()->Log("Game Initialization");
	...
	// [szczekus] add more flags if needed
	l_music1 = m_pSystem->GetISoundSystem()->LoadSound("Music/Loading/loading1.mp3",FLAG_SOUND_LOOP);
	l_music2 = m_pSystem->GetISoundSystem()->LoadSound("Music/Loading/loading2.mp3",FLAG_SOUND_LOOP);
	l_music3 = m_pSystem->GetISoundSystem()->LoadSound("Music/Loading/loading3.mp3",FLAG_SOUND_LOOP);
	return (true);
}
...
void CXGame::UpdateDuringLoading()
{
	...
	//m_pSystem->GetISoundSystem()->Update();
}
...
void CXGame::ProcessPMessages(const char *szMsg)
{
	if (!szMsg) 
		return;
	CryLogAlways("CXGame::ProcessPMessages(%s",szMsg,")");
	...
	{ 
		...
		// hardcoded values to fix the incorrect d3d9 renderer matrix
...
/*! Load a level on the local machine (connecting with a local client)
	...
	Called by ProcessPMessages
	Start of loading clean map
	+
	Called by LoadFromStream ( sometimes )
	Last stage of loading a saved game from file
*/
//////////////////////////////////////////////////////////////////////////
void CXGame::LoadLevelCS(bool keepclient, const char *szMapName, const char *szMissionName, bool listen)
{
	...
	m_pScriptTimerMgr->Reset();
	CryLogAlways("CXGame::LoadLevelCS called. %s", szMapName);
	
	/*
		[szczekus] 
		Play loading music if not played already
	*/
	if( !m_bIsLoadingLevelFromFile )//!this->l_music1->IsPlaying() && !this->l_music2->IsPlaying() && !this->l_music3->IsPlaying() )
	{
		srand(time(0));
		int x=rand() % 3+1;
