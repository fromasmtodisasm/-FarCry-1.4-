...
bool CXGame::SaveToStream(CStream &stm, Vec3d *pos, Vec3d *angles,string sFilename)
{
	...
	// [marco] save the number of entities, for the loading screen bar
	int nEnt=pEntitySystem->GetNumEntities();
	stm.Write(nEnt);

	WRITE_COOKIE_NO(stm,0x22);

	// [marco] save current EAX preset
	int nPreset;
  ...
  while((pEnt=pEntities->Next())!=NULL)
	...
	{
		// [PETAR] please do  not touch the next lines
...
bool CXGame::LoadFromStream(CStream &stm, bool isdemo)
{
	...
	CryLogAlways("CXGame::LoadFromStream called");
  ...
  //if(m_pSystem->GetISoundSystem())
	...
	{	// we`re loading a checkpoint from the same map we`re currently in from the "ingamesingle" screen
	...
	else // we`re loading a checkpoint from the same map we`re currently in, but from the "PageCampaignLoad" screen
	{	
		
		bLoadBar = 1;
		//szczekus: 5th loading texture are the entities, they take 90% of loading time anyway
		string sLoadingScreenTexture = m_currentLevelFolder + "/loadscreen_" + m_currentLevel + "5.dds";
		pConsole->SetLoadingImage( sLoadingScreenTexture.c_str() );

		pConsole->Clear();
		pConsole->ResetProgressBar(nEnt + 3 +20);
		pConsole->SetScrollMax(600);
		pConsole->ShowConsole(1);
		DeleteMessage("Switch"); // no switching during loading

		//CryLogAlways("Supposed progress bar range %i", nEnt);
	...
	{
		if (bLoadBar)
			pConsole->TickProgressBar();	// szczekus: advance progress
  ...
  if(nVersion<PATCH2_SAVEVERSION)
  {
	...
	// szczekus
	// There is no way of predict the exact progress bar length with 100% accuracy
	// not to mention the lack of a function returning current progress of the bar...
	// Tick progress bar to the end if it isn`t already
	for (int i = 0; i < nEnt*1.4f; i++)
	...
	//m_pSystem->GetISoundSystem()->Silence();// this is where the music should turn off
	this->l_music1->Stop();
	this->l_music2->Stop();
	this->l_music3->Stop();
	//CryLogAlways("We should be ingame now");
	GotoGame(1);
	m_nDEBUG_TIMING = 0;
...
{
	//CryLogAlways("CXGame::Load called");
	...
	if( x==2 )this->l_music2->Play(1,1,1);
	if( x==3 )this->l_music3->Play(1,1,1);
	//////////////////////////////////////////////////////////////////////////
	// Lock resources before loading checkpoint.
	// Speeds up loading a lot.
	...
	m_pSystem->GetIConsole()->SetScrollMax(300);
	m_pSystem->GetIConsole()->StaticBackground(false);
	//CryLogAlways("Crappy console loading screen returned to default state.");
