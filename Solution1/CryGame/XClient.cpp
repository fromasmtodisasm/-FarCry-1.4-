...
void CXClient::OnXContextSetup(CStream &stm)
{
	...
	//GetISystem()->GetINetwork()->Client_ReJoinGameServer();[szczekus]
	...
	// [szczekus] Don`t mute when we`re loading a save file
	//			 This function is called right before spawning all the entities
	if(m_pGame->GetSystem()->GetISoundSystem() && !m_pGame->m_bIsLoadingLevelFromFile)
	...
	if (!m_pGame->m_bIsLoadingLevelFromFile)
	{
		...
		m_pGame->l_music1->Stop();
		m_pGame->l_music2->Stop();
		m_pGame->l_music3->Stop();
	}
}
