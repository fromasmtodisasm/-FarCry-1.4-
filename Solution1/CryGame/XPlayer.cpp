...
bool CPlayer::Init()
{
	...
	// create the camera
	// <<FIXME>> create only for the client which does the rendering
	...
	// TIP: 67.5 * rw/rh for auto scaling with aspect ratio
	if( (rw/rh) > 1.34f )
		this->c_fov = DEG2RAD(90.0f); // Initialize with 90 degree fov.
	else
		this->c_fov = DEG2RAD(120.0f);// Initialize with 120 degree fov.

	pEntCamera->SetFov(this->c_fov,rw,rh);
	pEntCamera->GetCamera().Init(rw,rh,this->c_fov);
...
void CPlayer::Update()
{
	...
	//	in water checks, plus swimming states updates
...
void CPlayer::UpdatePhysics(float fDeltaTime)
{
	...
	// keep count of the last ground height
	if (!sl.bFlying)
	{
...
void CPlayer::ProcessAngles(CXEntityProcessingCmd &ProcessingCmd)
{
	...
	if (IsMyPlayer() && m_pVehicle && !m_bFirstPerson && !m_pMountedWeapon)
	{
		...
		// to mess around with the camera - he could switch
...
void CPlayer::ProcessMovements(CXEntityProcessingCmd &cmd, bool bScheduled)
{
		...
		if ( bNoGravity ) 
		{
			...
			if(m_bSwimming && m_pcos2>0)
			{
				if(	(m_stats.fInWater < m_PlayerDimSwim.heightEye && 
					...
					speedxyz[2] = m_pcos2*fWaterMoveScale;	// deep enough - can go up
			}
			else
				speedxyz[2] = m_pcos2;//*inputspeed;
		}
	...
	// if the velocity is too low - not sprinting then
	if(moveVel<2.0f)
		m_Sprinting = false;
...
void CPlayer::UpdateBonesRotation( )
{
	...
	if (m_LegRotation || m_stats.onLadder)
	{
		...
		float	zAngle = m_LegDeltaAngle;
			...
			{
				//force weapon (shoulders) be same direction as entity direction
				//m_LegADeltaLimit = 60;
...
void CPlayer::UpdateFireAnimations()
{
	...
	if( m_aimLook && m_stats.canfire )
	{
		...
		// if animation is shorter than blend time - it blends to base animation ( which is bad )
...
void CPlayer::StartAnimation( const SPlayerUpdateContext &ctx  )
{
	...
	if (m_JumpStage==1)
	{
		...
		//set jumpstage directly to 2 (in air)
		m_JumpStage = 2;
	}
